...
public class EditItemActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        borrower_spinner = (Spinner) findViewById(R.id.borrower);
        ...
        User[] userlist = new User[user_list.getSize()];
        for(int i = 0; i < user_list.getSize(); i++) {
            User u = user_list.getUser(i);
            userlist[i] = u;
        }
        ArrayAdapter spinnerArrayAdapter = new ArrayAdapter(this,android.R.layout.simple_spinner_item, userlist);
        borrower_spinner.setAdapter(spinnerArrayAdapter);

        String status_str = item.getStatus();
        if (status_str.equals("Borrowed")) {
            status.setChecked(false);
            borrower_spinner.setSelection(user_list.getIndex(item.getBorrower()));
        } else {
    ...
    public void saveItem(View view) {
        ...
        User borrower = (User)borrower_spinner.getSelectedItem();
        ...
        if (!checked) {
            ...
            Log.d("Borrower is set to ", borrower.getUsername());
        } else {
            updated_item.setBorrower(null);
        }
